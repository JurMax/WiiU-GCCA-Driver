...
- (Gcc *)setup:(int)ind {
    ...
    NSDictionary *properties = @{
                                 WJoyDeviceProductStringKey : (
                                                               [NSString stringWithFormat: @"Coole WiiU GCC Port  %@", @[@"1", @"2", @"3", @"4"] [ind]] ),
...
- (void) setup {
    ...
    if (dev_handle == NULL) {
        printf("WiiU GCC adapter not detected. Exiting...\n");
        return;
    }
...
- (void) testVoid: (NSTextField*) textField progressbar:(NSProgressIndicator*) ding {
    
    textField.enabled = FALSE;
    float seconds = textField.floatValue;
    
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        NSTimeInterval timeInMiliseconds = [[NSDate date] timeIntervalSince1970];
        double start = timeInMiliseconds;
        double current = 0;
        printf("Waiting [%f] seconds", seconds);
        while (current < seconds) {
            current = [[NSDate date] timeIntervalSince1970] - start;
            
            double progress = (current / seconds) * 100;
            dispatch_async(dispatch_get_main_queue(), ^{
                [ding setDoubleValue: progress];
            });
        }
        dispatch_async(dispatch_get_main_queue(), ^{
            [ding setDoubleValue: 100];
...
/*
 int mahjkin(int argc, const char * argv[]) {
 printf("ding started\n");
